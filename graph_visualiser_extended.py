...
class GraphVisualizer(tk.Tk):
    def __init__(self, graph):
        ...
        btn_cycle_undirected = tk.Button(control_frame, text="Detect Undirected Cycle",
                                         command=self.run_cycle_detection)
        btn_cycle_undirected.pack(side=tk.LEFT, padx=5)

        btn_cycle_directed = tk.Button(control_frame, text="Detect Directed Cycle",
            ...
            for nbr in self.graph.graph[v]:
                ...
                if self.graph.directed:
                    line_id = self.canvas.create_line(
                        start[0], start[1], end[0], end[1], arrow=tk.LAST, width=2
                    )
                else:
                    line_id = self.canvas.create_line(
                        start[0], start[1], end[0], end[1], width=2
                    )
    ...
    def reset_colors(self):
        for v, circle_id in self.vertex_circles.items():
    ...
    def run_cycle_detection(self):
        ...
        if self.graph.directed:
            self.info_label.config(text="Cycle detection for directed graph is not implemented.")
            return
